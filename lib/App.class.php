...
//    public static function Init()
    public static function Init($url = null)
    {
        ...
        */
        ...
        } else { // добавлено для тестирования
            self::web($url);
        }
    }
...
//    protected static function web($url)
    public static function web($url)
    {
        ...
        if (!empty($url[0])) { //Проверяем, установленна ли переменная
            $_GET['page'] = $url[0];
            if (!empty($url[1])) {
                if (is_numeric($url[1])) {
                    $_GET['id'] = $url[1];
        ...
        if (isset($_GET['page'])) {        //Проверим, установленна ли переменная
            ...
            /*
                        echo "Название класса контроллера: " . $controllerName;
                        echo "<br> Название метода контроллера: " . $methodName;
                        echo "<br> Название файла с классом контроллера: $controllerName.class.php";
            */
